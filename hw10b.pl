...
% Question 1: my_min  M is the minimum of X and Y
my_min(X,Y,X) :- X =< Y, write(X).
my_min(X,Y,Y) :- X > Y, write(Y).

% Question 2: my_median  where M is the median of the other three arguments
...
% Question 3: my_triangle_area Area given a base and height
my_triangle_area(Base,Height,Area) :- Area is Base*Height/2.


% Question 4: my_circle_area - circle area given a radius
my_circle_area(Radius,Area) :- Area is Radius*3.14*Radius.


% Question 5: my_midpoint - midpoint given a set of coordinates
my_midpoint((X1,Y1),(X2,Y2),(X3,Y3)) :- 
    X3 is ((X1+X2)/2), 
    Y3 is ((Y1+Y2)/2).


% Question 6: my_manhattan_distance - manhatton distance given two points
my_manhattan_distance((X1,Y1),(X2,Y2),D) :- D is abs(X1-X2) + abs(Y1-Y2).


% Question 7: my_euclidean_distance - euclidean distance given two points
my_euclidean_distance((X1,Y1),(X2,Y2),D) :- D is sqrt((X1-X2)*(X1-X2) + (Y1-Y2)*(Y1-Y2)).


% Question 8: my_range_sum - Find the sum of the range of numbers  x-y
my_range_sum(X,Y,S) :- 
    X < Y, 
    IncX is X + 1,
...
% Question 9: my_gcd - greatest common denominator of two numbers
